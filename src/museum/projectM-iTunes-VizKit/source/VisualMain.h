 ...
 * When displaying graphics, the visualizer spends most of its time with executing the render
 * message.
 */
class VisualMain
{
...
public:
	...
	static OSStatus VisualPluginHandler(
		OSType message, VisualPluginMessageInfo *visualPluginMessageInfo, void *refCon);
	 ...
	 * @param pluginMessageInfo A pointer to a data structure passed in by iTunes and sent on to
...
private:
	/** The constructor. VisualMain is a collection of static methods. Class does not need any
	 * instances. Constructor is private and not implemented. */
	VisualMain();

	/** The destructor. VisualMain is a collection of static methods. Class does not need any
	 ...
	 * @param isAudioStream True if track meta data belongs to audio stream. False if meta data
	 * belongs to audio track.
	 * @return True if audio track changed compared to previous one. False if metadata is the same as
	 * with the previous audio track.
	 */
	...
	/**
	 * Static helper function that converts a VisualPluginMessage to the string. Possibly useful for
 ...
 * On Mac the function is named iTunesPluginMainMachO for Mach-O plug-in. The function returns
 * OSStatus. On Windows the function is named iTunesPluginMain and declared with
 ...
 */
#if TARGET_OS_MAC
	__attribute__((visibility("default"))) extern OSStatus iTunesPluginMainMachO(
		OSType message, PluginMessageInfo *pluginMessageInfo, void *refCon);
#endif
#if TARGET_OS_WIN
	extern __declspec(dllexport) OSStatus
		iTunesPluginMain(OSType message, PluginMessageInfo *pluginMessageInfo, void *refCon);
#endif
