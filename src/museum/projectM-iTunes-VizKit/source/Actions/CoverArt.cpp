...
void CoverArt::prepareCoverAction()
{
	...
	double distance = this->coverArtAsset.getCamera().getMaxFarPos()
										- this->coverArtAsset.getCamera().getMaxNearPos();
	...
	VisualActorGraphics::translateMatrix(0.0, 0.0,
		distanceCenter - this->coverArtAsset.getCamera().getMaxNearPos() - (stageBoxDepth / 2.0));
}
...
void CoverArt::reshape()
{
	...
	coverArtAssetBox
		->update(); // VisualStageBox only updates automatically on value changed, after canvas reshape
								// event we have to update manually (before calculating scaleFactor)
