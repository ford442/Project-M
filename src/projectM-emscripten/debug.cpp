...
// printf("Start of renderFrame func (stm): %hh \n",stm);
...
lft=wave.slen;
wave.pos=0;
printf("End of bfr(while{:  stm=  %hh \n",stm);

}
SDL_memcpy(stm,wptr,len);
wave.pos+=len;
printf("End of bfr{:  stm=  %hh \n",stm);
}
void pl(){cls_aud();
char flnm[1024];
printf("Beginning of pl:  wave.snd=  %hh \n",wave.snd);
...
printf("After Loadwav:  &wave.snd=  %hh \n",&wave.snd);
