...
EM_JS(void,ma,(),{
let d=S();if(d)d();d=S();function S(){let vcc=document.getElementById("vcanvas");let w=document.getElementById("iwid").innerHTML;
let h=document.getElementById("ihig").innerHTML;w=Math.round(w);h=Math.round(h);let g=new GPU({canvas:vcc});
...
let $=new Uint8ClampedArray(W.buffer,0,l);$.set(t(v),0);r(t($));$.set(t(v),0);Kr.appendChild(r.vcc);r(t($));
$.set(t(v),0);let T=false;let ms=1;let R=16;let f=(1000/Rn);vcc=document.getElementById("vcanvas");function M(){if(T){return;}r(t($));$.set(t(v),0);
let mq=((ms*f)/R);let k=Math.floor(mq);let y=((k*f)-(k*Rn));if(y>8){R=8;}ms=ms+1;setTimeout(function(){M();},R);}M();
document.getElementById("di").onclick=function(){T=true;Kr.removeChild(r.vcc);t.destroy();r.destroy();g.destroy();
S();};return()=>{Kr.removeChild(r.vcc);T=true;};}});
int main(){
EM_ASM({
FS.mkdir('/presets');
FS.mkdir('/textures');
