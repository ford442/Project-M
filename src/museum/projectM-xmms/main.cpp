...
Uint32 get_xmms_title(Uint32 something, void *somethingelse)
{
	...
	// Nice optimization, but we want the title no matter what so I can display it when the song
	// changes
#if 0
	...
	{
		title = xmms_remote_get_playlist_title(projectM_vtable.xmms_session, pos);
...
int worker_func(void *)
{
	...
	while (SDL_SemValue(sem) == 1)
	{
		...
		//	 strcpy(title,xmms_remote_get_playlist_title(projectM_vtable.xmms_session,
		//xmms_remote_get_playlist_pos(projectM_vtable.xmms_session)));
...
extern "C" void projectM_render_pcm(gint16 pcm_data[2][512])
{
	...
	if (SDL_SemValue(sem) == 1) globalPM->pcm()->addPCM16(pcm_data);
