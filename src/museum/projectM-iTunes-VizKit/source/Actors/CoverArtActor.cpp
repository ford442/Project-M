...
void CoverArtActor::handleNotification(VisualNotification &aNotification)
{
	...
	switch (notificationKey)
	{
		...
		case kImageWithIdentifierMsg:
		{
			...
			{
				VisualImage *albumCoverArtworkImage =
					dynamic_cast<VisualImage *>(aNotification.getObject());
				...
				this->coverArt->fadeIn(
					VisualPreferences::getValue(VisualPreferences::kFadeInTimeOnPlayInMS));
			}
		}
		break;
		...
		case kAudioPlayPausedEvt:
			this->coverArt->fadeOut(
				VisualPreferences::getValue(VisualPreferences::kFadeOutTimeOnPauseInMS), 0.15f);
			break;

		case kAudioPlayResumedEvt:
			this->coverArt->fadeIn(
				VisualPreferences::getValue(VisualPreferences::kFadeInTimeOnResumeInMS));
			break;

		case kAudioPlayReachedFadeOutTimeEvt:
			this->coverArt->fadeOut(
				VisualPreferences::getValue(VisualPreferences::kFadeOutTimeBeforeEndOfTrackInMS));
			break;
