...
// printf("Start of renderFrame func (stm): %hh.X \n",stm);
...
lft=wave.slen;
wave.pos=0;
printf("End of bfr(while{:  stm=  %hh.X \n",stm);

}
SDL_memcpy(stm,wptr,len);
wave.pos+=len;
printf("End of bfr{:  stm=  %hh.X \n",stm);
}
void pl(){cls_aud();
char flnm[1024];
printf("Beginning of pl:  wave.snd=  %hh.X \n",wave.snd);
...
printf("After Loadwav:  &wave.snd=  %hh.X \n",&wave.snd);
