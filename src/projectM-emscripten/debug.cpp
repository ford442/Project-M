...
// printf("Start of renderFrame func (stm): %X.hh \n",stm);
...
lft=wave.slen;
wave.pos=0;
printf("End of bfr(while{:  stm=  %X.hh \n",stm);

}
SDL_memcpy(stm,wptr,len);
wave.pos+=len;
printf("End of bfr{:  stm=  %X.hh \n",stm);
}
void pl(){cls_aud();
char flnm[1024];
printf("Beginning of pl:  wave.snd=  %X.hh \n",wave.snd);
...
printf("After Loadwav:  &wave.snd=  %X.hh \n",&wave.snd);
