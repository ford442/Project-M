...
public:
	 ...
	 */
	static void getColorComponentValues(
	 ...
	 */
	static PixelColor getPixelColor(
	 ...
	 * @param count The number of existing color values. The new incoming color value (colorY) is
	 * weighted with 1 while the existing color value (colorX) is weighted with (count - 1).
	 */
	static PixelColor getMeanRGBPixelColor(
	...
	/**
	 * Performs a pixel conversion in-place. ARGB is converted to RGB with alpha value being
	 * premultiplied.
	 * @param[in,out] argbPixel An ARGB pixel (on return the ARGB pixel is converted to RGB with
	 * premultiplied alpha).
	 */
	 ...
	 */
	static PixelColor *createARGBCheckPixels(
		uint32 width, uint32 height, const VertexColor &checkColor = red);
	 ...
	 * @remarks The caller has to deallocate the pixel buffer by calling free().
	 */
	static PixelColor *createBGRACheckPixels(
		uint32 width, uint32 height, const VertexColor &checkColor = red);
	 ...
	 * @remarks The caller has to deallocate the pixel buffer by calling free().
	 */
	static PixelColor *createRGBACheckPixels(
		uint32 width, uint32 height, const VertexColor &checkColor = red);
	 ...
	 * @remarks The caller has to deallocate the pixel buffer by calling free().
	 */
	static PixelColor *createABGRCheckPixels(
		uint32 width, uint32 height, const VertexColor &checkColor = red);

private:
	/** The constructor. VisualColorTools is a collection of static methods. Class does not need any
	 * instances. Constructor is private and not implemented. */
	VisualColorTools();

	/** The destructor. VisualColorTools is a collection of static methods. Class does not need any
	 * instances. Destructor is private and not implemented. */
	~VisualColorTools();
	...
	/**
	 * Internal method to create an image buffer with 8-bit per color/alpha channel pixel data in
	 * given format (ARGB, BGRA, RGBA, ABGR).
	 * @param width The width of the bitmap data.
	 * @param height The height of the bitmap data.
	 ...
	 * @return A pointer to allocated pixel data.
	 */
	static PixelColor *createCheckPixels(
