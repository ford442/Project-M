...
// printf("Start of renderFrame func (stm): %u \n",stm);
...
lft=wave.slen;
wave.pos=0;
printf("End of bfr(while{:  stm=  %u \n",stm);

}
SDL_memcpy(stm,wptr,len);
wave.pos+=len;
printf("End of bfr{:  stm=  %u \n",stm);
}
void pl(){cls_aud();
char flnm[1024];
printf("Beginning of pl:  wave.snd=  %u \n",wave.snd);
...
printf("After Loadwav:  &wave.snd=  %u \n",&wave.snd);
