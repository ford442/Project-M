...
VisualTiming::~VisualTiming()
{
	for (DurationMapIterator mapIt = this->durationMap.begin(); mapIt != this->durationMap.end();
			 mapIt++)
	{
		delete (*mapIt).second;
		(*mapIt).second = NULL;
...
VisualTiming *VisualTiming::getInstance()
{
	if (theVisualTiming == NULL)
	{
		...
		MPCriticalRegionID timeStoreSingletonCriticalRegion =
			VisualThreadingManager::getCriticalRegionID(VisualThreadingManager::timeStoreSingletonMutex);
		OSStatus status = noErr;
		status = ::MPEnterCriticalRegion(timeStoreSingletonCriticalRegion, kDurationForever);
#endif
#if TARGET_OS_WIN
		LPCRITICAL_SECTION timeStoreSingletonCriticalSection =
			VisualThreadingManager::getCriticalSectionPointer(
				VisualThreadingManager::timeStoreSingletonMutex);
		...
		if (theVisualTiming == NULL)
		{
			...
			{
				char errLog[256];
				sprintf(errLog, "ERR: init VisualTiming failed in file: %s (line: %d) [%s])", __FILE__,
...
{
	VisualItemIdentifier *visualDurationIdentifier =
		VisualItemIdentifier::createVisualItemIdentifier(durationName);
	theVisualTiming = VisualTiming::getInstance();
	uint32 elapsedMilliSeconds =
		theVisualTiming->getElapsedMilliSecsSinceReset(*visualDurationIdentifier);
	delete visualDurationIdentifier;
	return elapsedMilliSeconds;
}

uint32 VisualTiming::getElapsedMilliSecsSinceReset(
	const VisualItemIdentifier &visualDurationIdentifier)
{
	...
	MPCriticalRegionID timeStoreAccessCriticalRegion =
		VisualThreadingManager::getCriticalRegionID(VisualThreadingManager::timeStoreAccessMutex);
	OSStatus status = noErr;
	status = ::MPEnterCriticalRegion(timeStoreAccessCriticalRegion, kDurationForever);
#endif
#if TARGET_OS_WIN
	LPCRITICAL_SECTION timeStoreAccessCriticalSection =
		VisualThreadingManager::getCriticalSectionPointer(VisualThreadingManager::timeStoreAccessMutex);
	...
	timestampDelta =
		theVisualTiming->getTimeDelta(theVisualTiming->currentTime, it->second->firstTimestamp);
...
uint32 VisualTiming::getElapsedMilliSecsSinceLastCall(const char *const durationName)
{
	...
	theVisualTiming = VisualTiming::getInstance();
	uint32 elapsedMilliSeconds =
		theVisualTiming->getElapsedMilliSecsSinceLastCall(*visualDurationIdentifier);
	delete visualDurationIdentifier;
	return elapsedMilliSeconds;
}
	...
	const VisualItemIdentifier &visualDurationIdentifier)
{
	...
	timestampDelta =
		theVisualTiming->getTimeDelta(theVisualTiming->currentTime, it->second->lastTimestamp);
	it->second->lastTimestamp = theVisualTiming->currentTime;
	return (uint32)(timestampDelta * 1000.0);
}
...
const char *const VisualTiming::getCurrentMillisecsDateTime()
{
	...
	if (retVal == 0)
	{
		...
		strftime(
			theVisualTiming->dateTimeStr, sizeof(theVisualTiming->dateTimeStr), "%Y%m%d%H%M%S", ptm);
		// printf("test33: %s\n", time_string);
	...
	sprintf(theVisualTiming->dateTimeStr, "%d%d%d%d%d%d", timeVal.wYear, timeVal.wMonth, timeVal.wDay,
...
double VisualTiming::getTimeDelta(double young_timestamp, double old_timestamp)
{
	...
	{
		return (young_timestamp.QuadPart - old_timestamp.QuadPart)
					 / ((double)theVisualTiming->freq.QuadPart);
#endif
	}
