...
typedef enum
{
	kNoLoop = 0x0, /**< 00000000 - After the duration of the timeline has been reached, the animation
										stops. */
	kLoop =
		0x2, /**< 00000010 - After the duration of the timeline has been reached, the timeline starts
						anew at the start. The timeline is always processed in forward direction. */
	kMirroredLoop =
		0x3 /**< 00000011 - After the duration of the timeline has been reached, the timeline is
					 processed in backward direction. The moving direction toggles in mirrored loop. */
} LoopMode;

const sint32 kInfiniteRepetition = -1;

/**
 * A VisualAnimation is constructed with an AnimatedProperty. With VisualActor::addAnimation()
 * specific properies of an asset can be animated. A provided callback function is called for the
 * time of the duration of the animation. The animation tries to reach the stop value in the amount
 * of time set by the duration value.
 */
...
public:
	 ...
	 */
	void setAnimateCallbackFunctionPtr(
		VisualAnimationAnimateCallback theCallback, void *someUserData = NULL);

	/**
	 * Sets the identifier of the parent collection (the collection which the current animation is
	 * part of).
	 ...
	 * @remarks Only appropriate for collection (like cluster and sequence). VisualAnimation simply
	 * returns after printing an error message.
	 */
	 ...
	 * @remarks Return value can be NULL if no VisualAnimation with the specified VisualItemIdentifier
	 * is found.
	 */
	virtual VisualAnimationComponent *getVisualAnimation(
		const VisualItemIdentifier &anAnimationIdentifier);
	 ...
	 * @remarks In case the start is delayed, the animation is only running after the delaying
	 * milliseconds did have passed.
	 */
	 ...
	 * @remarks The animation always ranges from start value to stop value (default: 0.0 to 1.0).
	 * Setting a specific current value here means that the animation starts its first step with the
	 * specific current value. Can be useful if an animation is supposed to continue at the point in
	 * time of a prior animation. The initial value of the constructor (0.0) does not change the
	 * current value.
	 */
	 ...
	 * @param startAnimationWithCurrentPropertyValue If true, the animation starts with the current
	 * value of the animated property. Default is false as it is assumed that a set start value is
	 * meant to act as value to start the animation with.
	 ...
	 */
	void setStartValue(
		const VisualStagePosition &aPosition, bool startAnimationWithCurrentPropertyValue = false);
	 ...
	 * @param requestedNumberOfLoops The number of times the animation should be repeated (default =
	 * 1).
	 */
	 ...
	 * @remarks The delay is set when calling setDurationInMilliseconds(). The value is used by
	 * VisualStageControl.
	 */
	 ...
	 * @param aStartDelayInMilliseconds The number of milliseconds until the animation actually
	 * starts.
	...
	/**
	 * Sets the callback function that is repeatedly called back during animation component's duration
	 * time.
	 * @param theCallback Pointer to c-style function (static class function).
	...
	virtual void setCallbackToPerformAnyAdditionalAction(
		VisualAnimationPerformAnyAdditionalActionCallback theCallback, void *userData = NULL);
	 ...
	 * @param userData Additional data (e.g. pointer to instance variable of initialized class).
	 */
	virtual void setCallbackToNotifyBeforeDeath(
		VisualAnimationWillDieCallback theCallback, void *userData = NULL);
	 ...
	 * @remarks Called by VisualAnimationQueue. Updates the current value of the animation. Also tests
	 * whether the animation is supposed to be started in case the start was delayed.
	 */
	 ...
	 * @remarks The animation is redy to die when the duration is exceeded. VisualAnimationQueue acts
	 * on a true return value and deletes the VisualAnimation.
	 */
	 ...
	 * @param animationSpeed The speed of the animation (as it has been calculated by static function
	 * VisualAnimation::calcSpeed());
	 */
	...
	/**
	 * Calculates the speed of an animation if it would use the passed start and stop values with the
	 * provided duration time.
	 ...
	 */
	static AnimationSpeed calcSpeed(
		double aStartValue, double aStopValue, uint32 aDurationInMilliseconds);
	...
	/**
	 * Calculates the time an animation will take to travel a certain distance with a certain constant
	 * speed.
	 * @param aDistance The distance of the values of the animation.
...
private:
	...
	/**
	 * Internal method to adjust the current timeline value to the current value of the animated
	 * property.
	 * @param visualAsset The asset where a property is animated.
	 * @param anAnimatedProperty The animated property.
	 */
	void setCurrentTimelineValueToCurrentPropertyValue(
		const VisualAsset &visualAsset, AnimatedProperty anAnimatedProperty);
	...
	/** Internally stored pointer to provided user data (e.g. pointer to instance variable of
	 * initialized class). */
	...
	/** The optional user data that is passed to callback function that is called during the
	 * animation. */
	...
	static const uint32
		maxNumberOfDebugHistoryEntries; /**< The maximum number of data entries stored in history for
																			 debug and monitoring purposes. */
	...
	/** The optional identifier of the collection of the animation (the cluster resp. sequence the
	 * current animation is part of). */
	...
	/** The start value of the animation as it has been set by setStartValue(), with data type
	 * VisualStagePosition. */
	...
	/** The stop value of the animation as it has been set by setStopValue(), with data type
	 * VisualStagePosition. */
	...
	/** The optional user data that is passed to callback function that is called right before the
	 * death of the animation cluster. */
