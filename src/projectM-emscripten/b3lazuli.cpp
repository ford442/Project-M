...
let $=new Uint8ClampedArray(W.buffer,0,l);$.set(t(v),0);r(t($));$.set(t(v),0);k.appendChild(r.vcan);r(t($));$.set(t(v),0);let T=false;let ms=1;let R=16;let f=(1000/Rn);let vcan=document.getElementById("vcanvas");vcan.style.cssText="image-rendering:pixelated;";function M(){if(T){return;}r(t($));$.set(t(v),0);let mq=((ms*f)/R);let k=Math.floor(mq);let y=((k*f)-(k*Rn));if(y>8){R=8;}ms=ms+1;setTimeout(function(){M();},R);}M();document.getElementById("di").onclick=function(){T=true;k.removeChild(r.vcan);t.destroy();r.destroy();g.destroy();S();};return()=>{k.removeChild(r.vcan);T=true;};}});
