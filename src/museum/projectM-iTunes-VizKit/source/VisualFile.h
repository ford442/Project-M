...
public:
	 ...
	 * @return True on success, false on failure.
	 * @remarks The memory has to be deallocated on the caller's side by calling free() on the
	 * returned buffer.
	 */
	 ...
	 * @remarks The caller has to release the allocated memory by calling delete() on the returned
	 * VisualFile pointer.
	 */
	 ...
	 * @return True on success, false on failure.
	 * @remarks Windows-only. On the Mac resources are stores are regular files in Resurces directory
	 * of bundle.
	 */
...
private:
	 ...
	 * @param[in,out] numberOfbytes The number of bytes requested. The actual number of bytes written
	 * on return.
