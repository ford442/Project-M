...
public:
	...
	/**
	 * A constructor that constructs a VisualString with a pointer to an UTF-8 string and the number
	 ...
	 * @remarks If the number of bytes is not specified, the string must be terminated with a NULL
	 * byte. The number of bytes is evaluated by calling strlen() then.
	 */
	VisualString(const char *const aStringVal, sint32 numberOfBytes = -1);

	/**
	 * A constructor that constructs a VisualString with a pointer to a wchar_t string and the number
	 ...
	 * @remarks If the number of characters is not specified, the string must be terminated with a
	 * NULL byte. The number of characters is evaluated by calling sizeof() then.
	 */
	 ...
	 * @remarks ConstStringDataRef is the cross-platform data format of VisualString's internal string
	 * buffer.
	 */
	 ...
	 * @remarks The file is opened, the content is read as an UTF-8 encoded string, then the file is
	 * closed again.
	 */
	...
	/**
	 * Initializes a VisualString with a platform native string data type (CFStringRef on Mac, wchar_t
	 * pointer on Windows).
	 ...
	 * Less than operator.
	 * @param other Another VisualString.
	 * @remarks The less than operator is used by std::map for find() and sorting (if VisualString is
	 * used as key_type).
	 */
	 ...
	 * @remarks The charString must be NULL-terminated so that strlen can determine the correct length
	 * of the string.
	 */
	 ...
	 * @remarks The number of characters are not the number of bytes in memory but the number of
	 * glyphs or code points.
	 */
	 ...
	 * @remarks The number of lines is evaluated by counting the number of lines that contain at least
	 ...
	 * @remarks On Mac the string data is stored as a CFString. On Windows the string data is stored
	 * as a wchar_t type.
	 */
	 ...
	 * @remarks The memory is owned by VisualString and deallocated on destruct of VisualString
	 * instance.
	 */
	 ...
	 * @remarks Line endings can be incorrect when transfering audio files from MacOS to Windows
	 * (Windows expects line ending to consist of CR+LF).
	 */
	...
	/**
	 * Joins the strings of the vector to create a joined string. The items of the vector are
	 ...
	 * @param aStringVal A pointer to the UTF-8 encoded char string value.
	 * @remarks The string must be terminated with a NULL byte. The caller has to delete the
	 * VisualString.
	 */
	 ...
	 * @remarks The caller has to delete the VisualString.
	 */
	static VisualString *createWithUTF16Buffer(
	 ...
	 * @param anId An identifier.
	 */
	static VisualString *createWithContentsOfURL(
		const char *const aURL, const VisualItemIdentifier &anId);

	/**
	 * Creates a VisualString with a platform native string data type (CFStringRef on Mac, wchar_t
	 * pointer on Windows).
	 * @param constStringDataRef On Mac a CFStringRef, on Windows a wchar_t pointer.
	 */
