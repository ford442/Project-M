...
void VisualColorTools::getColorComponentValues(
	const PixelColor &pixelColor, uint8 &alpha, uint8 &red, uint8 &green, uint8 &blue)
{
...
PixelColor VisualColorTools::getPixelColor(
...
void VisualColorTools::getMeanPixelColor(
	const PixelColor *colors, uint32 count, PixelColor &avgPixelColor)
{
	...
	for (uint32 i = 0; i < count; i++)
	{
		...
		avg_ch_1 =
			(avg_ch_1 * (double)loopCount + (double)((colors[i] >> 24) & 0xff)) / (double)(loopCount + 1);
		avg_ch_2 =
			(avg_ch_2 * (double)loopCount + (double)((colors[i] >> 16) & 0xff)) / (double)(loopCount + 1);
		avg_ch_3 =
			(avg_ch_3 * (double)loopCount + (double)((colors[i] >> 8) & 0xff)) / (double)(loopCount + 1);
		avg_ch_4 =
			(avg_ch_4 * (double)loopCount + (double)(colors[i] & 0xff)) / (double)(loopCount + 1);

		loopCount++;
	}

	avgPixelColor =
		((uint8)avg_ch_1 << 24) | ((uint8)avg_ch_2 << 16) | ((uint8)avg_ch_3 << 8) | (uint8)avg_ch_4;
}

PixelColor VisualColorTools::getMeanRGBPixelColor(
	const PixelColor &colorX, const PixelColor &colorY, const uint32 &count)
{
	...
	// VisualColorTools::rgb2hsl((double)r_x / (double)255.0, (double)g_x / (double)255.0, (double)b_x
	// / (double)255.0, h_x, s_x, l_x); VisualColorTools::rgb2hsl((double)r_y / (double)255.0,
	// (double)g_y / (double)255.0, (double)b_y / (double)255.0, h_y, s_y, l_y);
...
void VisualColorTools::RGBToHSL(
	const double &r, const double &g, const double &b, double *h, double *s, double *l)
{
...
void VisualColorTools::HSLToRGB(
	const double &h, const double &s, const double &l, double *r, double *g, double *b)
{
...
void VisualColorTools::RGBToHSV(
	const double &r, const double &g, const double &b, double *h, double *s, double *v)
{
...
void VisualColorTools::HSVToRGB(
	const double &h, const double &s, const double &v, double *r, double *g, double *b)
{
...
PixelColor *VisualColorTools::createARGBCheckPixels(
	uint32 width, uint32 height, const VertexColor &checkColor)
{
	return VisualColorTools::createCheckPixels(width, height, "ARGB", checkColor);
}

uint32 *VisualColorTools::createBGRACheckPixels(
	uint32 width, uint32 height, const VertexColor &checkColor)
{
	return VisualColorTools::createCheckPixels(width, height, "BGRA", checkColor);
}

uint32 *VisualColorTools::createRGBACheckPixels(
	uint32 width, uint32 height, const VertexColor &checkColor)
{
	return VisualColorTools::createCheckPixels(width, height, "RGBA", checkColor);
}

uint32 *VisualColorTools::createABGRCheckPixels(
	uint32 width, uint32 height, const VertexColor &checkColor)
{
	return VisualColorTools::createCheckPixels(width, height, "ABGR", checkColor);
}

PixelColor *VisualColorTools::createCheckPixels(
	uint32 width, uint32 height, const char *const format, const VertexColor &checkColor)
{
	...
	{
		char errStr[256];
		sprintf(errStr, "Err: unknown colorChannelFormat in file: %s (line: %d) [%s])", __FILE__,
	...
	uint8 color = 0;
	if (checkColor.r == 1.0 && checkColor.g == 0.0 && checkColor.b == 0.0 && checkColor.a == 1.0
			&& checkColor.red == 1.0 && checkColor.green == 0.0 && checkColor.blue == 0.0
			&& checkColor.alpha == 1.0)
	{
		color = 1; // red
	}
	else if (checkColor.r == 0.0 && checkColor.g == 1.0 && checkColor.b == 0.0 && checkColor.a == 1.0
					 && checkColor.red == 0.0 && checkColor.green == 1.0 && checkColor.blue == 0.0
					 && checkColor.alpha == 1.0)
	{
		color = 2; // green
	}
	else if (checkColor.r == 0.0 && checkColor.g == 0.0 && checkColor.b == 1.0 && checkColor.a == 1.0
					 && checkColor.red == 0.0 && checkColor.green == 0.0 && checkColor.blue == 1.0
					 && checkColor.alpha == 1.0)
	{
		color = 3; // blue
	}
	else if (checkColor.r == 1.0 && checkColor.g == 1.0 && checkColor.b == 1.0 && checkColor.a == 1.0
					 && checkColor.red == 1.0 && checkColor.green == 1.0 && checkColor.blue == 1.0
					 && checkColor.alpha == 1.0)
	{
		color = 4; // white
	}
	else
	{
		char errStr[256];
		sprintf(
			errStr, "unknown checkColor in file: %s (line: %d) [%s])", __FILE__, __LINE__, __FUNCTION__);
		writeLog(errStr);
		return NULL;
	}
	...
	if (pixels == NULL)
	{
		char errStr[256];
		sprintf(
			errStr, "pixels == NULL in file: %s (line: %d) [%s])", __FILE__, __LINE__, __FUNCTION__);
		writeLog(errStr);
		return pixels;
	}
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++)
		{
			...
			{
				case 1: pixels[cnt] = (a << 24) | (r << 16) | (g << 8) | b; break;
				case 2: pixels[cnt] = (b << 24) | (g << 16) | (r << 8) | a; break;
				case 3: pixels[cnt] = (r << 24) | (g << 16) | (b << 8) | a; break;
				case 4: pixels[cnt] = (a << 24) | (b << 16) | (g << 8) | r; break;
				default:
					char errStr[256];
					sprintf(errStr, "unknown colorChannelFormat in file: %s (line: %d) [%s])", __FILE__,
						__LINE__, __FUNCTION__);
					writeLog(errStr);
			}
			cnt++;
