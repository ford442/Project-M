...
void VisualString::initWithUTF8String(const char *const aStringVal)
{
	...
	if (aStringVal != NULL)
	{
		...
		CFStringRef theStringRef = CFStringCreateWithBytes(kCFAllocatorDefault,
			(const uint8 *)aStringVal, numberOfBytes, kCFStringEncodingUTF8, isExternalRepresentation);
		...
		int requiredSize =
			MultiByteToWideChar(CP_UTF8, dwFlags, aStringVal, numberOfBytes, theString, destinationSize);
		if (requiredSize > 0)
		{
			...
			int sizeConverted = MultiByteToWideChar(
				CP_UTF8, dwFlags, aStringVal, numberOfBytes, theString, destinationSize);
...
bool VisualString::operator<(const VisualString &other) const
{
	...
	bool VisualString::operator==(const VisualString &other) const
	{
		...
		bool VisualString::operator!=(const VisualString &other) const { return !(*this == other); }
		...
		void VisualString::copy(const VisualString &other)
		{
			...
			if (other.utf8representation != NULL)
			{
				...
				memcpy(
					this->utf8representation, other.utf8representation, strlen(other.utf8representation));
			}
			else
			{
			...
			{
				this->utf16representation =
					(uint16 *)malloc(sizeof(other.utf16representation) * sizeof(char));
				memcpy(
					this->utf16representation, other.utf16representation, sizeof(other.utf16representation));
			}
			else
			{
		...
		VisualString *VisualString::clone(void) const { return new VisualString(*this); }

		uint32 VisualString::getNumberOfCharacters() const { return this->numberOfCharacters; }
		...
		uint32 VisualString::getNumberOfWords(void) const { return this->numberOfWords; }

		uint32 VisualString::getNumberOfLines(void) const { return this->numberOfLines; }
...
#if TARGET_OS_WIN
			stringAtLine =
				new VisualString((uint16 *)&this->theString[this->arrayOfLines[index].startIndex],
					this->arrayOfLines[index].numberOfCharacters);
#endif
#if TARGET_OS_MAC
			CFStringRef string = CFStringCreateWithSubstring(kCFAllocatorDefault, this->theString,
				CFRangeMake(
					this->arrayOfLines[index].startIndex, this->arrayOfLines[index].numberOfCharacters));
...
#if TARGET_OS_WIN
			wordAtIndex =
				new VisualString((uint16 *)&this->theString[this->arrayOfWords[index].startIndex],
					this->arrayOfWords[index].numberOfCharacters);
#endif
#if TARGET_OS_MAC
			CFStringRef string = CFStringCreateWithSubstring(kCFAllocatorDefault, this->theString,
				CFRangeMake(
					this->arrayOfWords[index].startIndex, this->arrayOfWords[index].numberOfCharacters));
		...
		ConstStringDataRef VisualString::getCharactersPointer() const { return this->theString; }
		...
		const char *const VisualString::getUtf8Representation()
		{
			...
			if (this->theString != NULL)
			{
				...
				numberOfCharsConverted = CFStringGetBytes(this->theString, range, kCFStringEncodingUTF8,
					lossByte, isExternalRepresentation, NULL, maxBufLen, &numberOfBytes);
				if ((numberOfCharsConverted == CFStringGetLength(this->theString)) && (numberOfBytes > 0))
				{
					...
					numberOfCharsConverted = CFStringGetBytes(this->theString, range, kCFStringEncodingUTF8,
						lossByte, isExternalRepresentation, (uint8 *)this->utf8representation, numberOfBytes,
						&numberOfBytes);
			...
			/*
				const char* scriptPathConstCStr =
				CFStringGetCStringPtr(scriptFilePath.getCharactersPointer(), kCFStringEncodingUTF8); char*
				scriptPathCStr = NULL; if (scriptPathConstCStr == NULL) { CFIndex stringLength =
				CFStringGetLength(scriptFileSystemPath); CFIndex maxSize =
				CFStringGetMaximumSizeForEncoding(stringLength, kCFStringEncodingUTF8); scriptPathCStr =
				(char*)malloc((maxSize + 1) * sizeof(char)); bool success =
				CFStringGetCString(scriptFileSystemPath, scriptPathCStr, maxSize, kCFStringEncodingUTF8); if
			...
			*/
			...
			int numberOfBytesRequired =
				WideCharToMultiByte(CP_UTF8, dwFlags, this->theString, this->numberOfCharacters,
					this->utf8representation, numberOfDestinationBytes, lpDefaultChar, lpUsedDefaultChar);
		...
		VisualString VisualString::getSubstring(uint32 startIdx, uint32 numberOfSubstringCharacters)
			const
		{
			...
			for (uint32 i = startIdx; i < (startIdx + numberOfSubstringCharacters); i++)
			{
				...
				VisualString *character =
					VisualString::createWithUTF16Buffer(&characterAtIndex, numberOfUTF16Characters);
				if (character)
				{
		...
		void VisualString::normalizeLineEndings()
		{
			...
			this->initWithUTF16Buffer(
				normalizedString->getUtf16Representation(), normalizedString->getNumberOfCharacters());
...
#if TARGET_OS_MAC
			CFCharacterSetRef whitespaceAndNewlineSet =
		...
		std::vector<VisualString> VisualString::splitWithDelimiter(const VisualString &delimiter)
		{
			...
			this->getCharactersPointer(), delimiter.getCharactersPointer()); for (CFIndex i = 0; i <
			CFArrayGetCount(array); i++) { CFStringRef stringRef =
			(CFStringRef)CFArrayGetValueAtIndex(array, i); CFIndex length = CFStringGetLength(stringRef);
		...
		VisualString *VisualString::createJoinedString(std::vector<VisualString> stringVector)
		{
			...
			VisualString *returnString =
				VisualString::createWithUTF8String(string.getUtf8Representation());
			return returnString;
		}
		...
		void VisualString::calcStatistics()
		{
			...
			if (this->characterIsNewlineCharacter(this->getCharacterAtIndex(this->numberOfCharacters - 1))
					== false)
			{
				visualStringSubstringPointer.startIndex = lineStartPos;
				visualStringSubstringPointer.numberOfCharacters = numberOfLineCharsScanned;
			...
			if (this->characterIsWhitespaceCharacter(
						this->getCharacterAtIndex(this->numberOfCharacters - 1))
					== false)
			{
				visualStringSubstringPointer.startIndex = wordStartPos;
				visualStringSubstringPointer.numberOfCharacters = numberOfWordCharsScanned;
		...
		VisualString *VisualString::createWithUTF8Buffer(
			const char *const aStringVal, uint32 numberOfBytes)
		{
		...
		VisualString *VisualString::createWithUTF16Buffer(
			const uint16 *const aStringVal, uint32 aNumberOfCharacters)
		{
		...
		VisualString *VisualString::createWithContentsOfURL(
			const char *const aURL, const VisualItemIdentifier &anId)
		{
			VisualString *aVisualString = new VisualString;
