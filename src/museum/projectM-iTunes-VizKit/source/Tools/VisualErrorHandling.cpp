...
void writeLog(const char *const infoStr)
{
	...
	if (textFile)
	{
		...
		VizKit::VisualString logFileName =
			VizKit::VisualString(VizKit::VisualConfiguration::visualizerPluginIdentifierName);
	...
	else
	{
		...
		hSource =
			RegisterEventSource(NULL, VizKit::VisualConfiguration::visualizerPluginIdentifierName);
		if (hSource != NULL)
		{
...
void printframeinfo(unsigned int level, void *fp, void *ra)
{
	...
	// size_t length;
	// int status;
	// char* demangledNameRetChar = abi::__cxa_demangle(info.dli_sname, demangledName, &length,
	// &status); printf("demangled: %s\n", demangledName);
...
void addSourceToRegistry(char *pszAppname, char *pszMsgDLL)
{
	...
	bSuccess = RegCreateKey(
		HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\NTP", &hk);
