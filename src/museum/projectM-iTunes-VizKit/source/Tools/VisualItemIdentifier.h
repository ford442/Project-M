 ...
 * The internal token of the identifier stays the same across copy operations. Comparisons of copied
 * instances yield equal result. Identifiers can be shared by supplying a name. If another
 * VisualItemIdentifier with the same name has been created already, it is returned by
 * VisualItemIdentifierRegistry's getVisualItemIdentifier().
 */
...
public:
	 ...
	 * Less than operator.
	 * @param other Another VisualItemIdentifier.
	 * @remarks The less than operator is used by std::map for find() and sorting (if
	 * VisualItemIdentifier is used as key_type).
	 */
	...
	/**
	 * Returns a VisualItemIdentifier identified by name. By using a name as token, the same
	 * identifier can be used at different places.
...
private:
	 ...
	 * @remarks The internal identifier stays the same across copies and instances. Since it is typed
	 * as 32bit unsigned integer the number of identifiers is limited to 4294967296.
	 */
	uint32 token;
	...
	/** A VisualItemIdentifierNameTokenMapIterator is an iterator of a
