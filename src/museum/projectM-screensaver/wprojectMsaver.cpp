...
{
	// Under NT, we return true immediately. This lets the saver quit, and the system manages
	// passwords. Under '95, we call VerifyScreenSavePwd. This checks the appropriate registry key
	// and, if necessary, pops up a verify dialog
	OSVERSIONINFO osv;
	...
	switch (msg)
	{
		...
		case WM_MOUSEMOVE:
		{
			if (ScrMode == smSaver && !ss->IsDialogActive)
			{
				...
				if (dy < 0) dy = -dy;
				if (dx > (int)ss->MouseThreshold || dy > (int)ss->MouseThreshold) ss->CloseSaverWindow();
			}
		}
		break;
		case (WM_SYSCOMMAND):
		...
		case (WM_CLOSE):
		{
			...
			if (ScrMode == smSaver)
				return false; // so that DefWindowProc doesn't get called, because it would just
											// DestroyWindow
		}
		break;
		case (WM_DESTROY):
		{
			if (ss->idTimer != 0) KillTimer(hwnd, ss->idTimer);
			ss->idTimer = 0;
			PostQuitMessage(0);
		}
		break;
...
void DoSaver(HWND hparwnd, int nCmdShow)
{
	...
	if (ScrMode == smPreview)
	{
		...
		cx = rc.right - rc.left;
		cy = rc.bottom - rc.top;
		hScrWindow = CreateWindow(
			"OpenGL", "SaverWindow", WS_CHILD | WS_VISIBLE, 0, 0, cx, cy, hparwnd, NULL, hInstance, NULL);
	}
	else
	{
		...
		hScrWindow =
			CreateWindow("OpenGL", "SaverWindow", style, 0, 0, cx, cy, NULL, NULL, hInstance, NULL);
	}
...
int WINAPI WinMain(HINSTANCE h, HINSTANCE, LPSTR, int nCmdShow)
{
	...
	// We create a global TSaverSettings here, for convenience. It will get used by the config dialog
	// and by the saver as it runs
