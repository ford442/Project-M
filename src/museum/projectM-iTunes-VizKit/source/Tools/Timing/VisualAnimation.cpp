...
void VisualAnimation::copy(const VisualAnimation &other)
{
	...
	this->performAnyAdditionalActionCallbackUserData =
		other.performAnyAdditionalActionCallbackUserData;
...
void VisualAnimation::setAnimateCallbackFunctionPtr(
	VisualAnimationAnimateCallback theCallback, void *someUserData)
{
	this->animateCallback = theCallback;
	this->animationUserData = someUserData;
}

void VisualAnimation::setIdentifierOfParentCollection(
	const VisualItemIdentifier &aCollectionIdentifier)
{
	if (this->collectionIdentifier != NULL)
	{
		delete this->collectionIdentifier;
...
void VisualAnimation::handleOneCollectionItemAnimationDied()
{
	char errLog[256];
	sprintf(errLog,
		"Err: VisualAnimation is unable to handle collection animations in file: %s (line: %d) [%s])",
		__FILE__, __LINE__, __FUNCTION__);
	writeLog(errLog);
	return;
}

VisualAnimationComponent *VisualAnimation::getVisualAnimation(
...
void VisualAnimation::animate()
{
	...
	if (this->debugMode == true)
	{
		...
		VisualGraphics::drawHistoryDiagram(
			this->debugHistory, this->currDebugHistoryEntry, 0.0, 1.0, *aCamera);
		delete aCamera;
	...
	{
		this->performAnyAdditionalActionCallback(
			this, this->performAnyAdditionalActionCallbackUserData);
	}
}
...
void VisualAnimation::stop()
{
	...
	if (this->collectionIdentifier != NULL)
	{
		VisualAnimationComponent *animationCollection =
			VisualAnimationQueue::getVisualAnimation(*(this->collectionIdentifier));
		...
		{
			char errLog[256];
			sprintf(errLog, "CollectionAnimation not found in in file: %s (line: %d) [%s])", __FILE__,
...
void VisualAnimation::setDurationInMilliseconds(uint32 numberOfMilliseconds)
{
	...
	this->speed = this->calcSpeed(
		this->theTimeline->getDistance(), this->theTimeline->getDurationInMilliseconds());
...
void VisualAnimation::setCurrentValue(double aCurrentValue)
{
	...
	{
		// movingDirection might need to change (we want to arrive at the stop position by way of the
		// shortest distance)
...
void VisualAnimation::setStartValue(
	const VisualStagePosition &aPosition, bool startAnimationWithCurrentPropertyValue)
{
	AnimationSpeed prevSpeed = this->getSpeed();
	// this->theTimeline->setStartValue(aStartValue);
	// this->startValue = aStartValue;
...
void VisualAnimation::setCallbackToPerformAnyAdditionalAction(
	VisualAnimationPerformAnyAdditionalActionCallback theCallback, void *userData)
{
	this->performAnyAdditionalActionCallback = theCallback;
	this->performAnyAdditionalActionCallbackUserData = userData;
}

void VisualAnimation::setCallbackToNotifyBeforeDeath(
	VisualAnimationWillDieCallback theCallback, void *userData)
{
...
void VisualAnimation::update()
{
	if (this->status == kIsNotRunning)
	{
		if (this->startDelayInMilliseconds > 0)
		{
			uint32 elapsedMillisecondsOfDelay =
				VisualTiming::getElapsedMilliSecsSinceReset(VisualObject::getIdentifier());
...
void VisualAnimation::preparePriorToAddingToAsset(VisualAsset &visualAsset)
{
	...
	VisualAnimationQueue::removeVisualAnimationsWithOwnerIdentifier(
		visualAsset.getIdentifier(), this->animatedProperty);
...
void VisualAnimation::setSpeed(AnimationSpeed animationSpeed)
{
	...
	if (this->debugMode == true)
	{
		char logStr[128];
		sprintf(
			logStr, "VisualAnimation::setSpeed: newDuration: %d (distance: %f)", newDuration, distance);
		writeLog(logStr);
	}
	this->theTimeline->setDurationInMilliseconds(newDuration);
	/*
	if (movingDirection == kAscending) {
		timelineStartValue = theTimeline->getMaxValue() -
	(this->theTimeline->getDurationInMilliseconds() * animationSpeed); } else if (movingDirection ==
	kDescending) { timelineStartValue = theTimeline->getMinValue() +
	(this->theTimeline->getDurationInMilliseconds() * animationSpeed);
	}
...
AnimationSpeed VisualAnimation::calcSpeed(
...
void VisualAnimation::setCurrentTimelineValueToCurrentPropertyValue(
	const VisualAsset &visualAsset, AnimatedProperty anAnimatedProperty)
{
	...
	{
		adjustedDistanceAndDuration =
			this->theTimeline->setCurrentValue(visualAsset.getCurrentAnimationValueForAnimatedLocation(
				this->startValueVisualStagePosition, this->stopValueVisualStagePosition));
	}
	else
	{
		adjustedDistanceAndDuration = this->theTimeline->setCurrentValue(
			visualAsset.getCurrentAnimationValueOfProperty(anAnimatedProperty));
	}
	// movingDirection might need to change (we want to arrive at the stop position by way of the
	// shortest distance)
	MovingDirection movingDirection = this->theTimeline->getMovingDirection();
