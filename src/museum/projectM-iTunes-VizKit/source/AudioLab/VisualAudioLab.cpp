...
void VisualAudioLab::init()
{
	...
	{
		waveformData[i] =
			(sint8 **)malloc(VisualAudioLab::maxNumberOfAudioDataChannels * sizeof(sint8 *));
	}
	for (i = 0; i < VisualAudioLab::maxNumberOfMusicDataHistory; i++)
	{
		for (k = 0; k < VisualAudioLab::maxNumberOfAudioDataChannels; k++)
		{
			waveformData[i][k] =
				(sint8 *)malloc(VisualAudioLab::numberOfAudioWaveformDataEntries * sizeof(sint8));
	...
	// waveformDataMono
	waveformDataMono =
		(sint16 **)malloc(VisualAudioLab::maxNumberOfMusicDataHistory * sizeof(sint16 *));
	for (i = 0; i < VisualAudioLab::maxNumberOfMusicDataHistory; i++)
	{
		waveformDataMono[i] =
			(sint16 *)malloc(VisualAudioLab::numberOfAudioWaveformDataEntries * sizeof(sint16));
	...
	{
		spectrumData[i] =
			(uint8 **)malloc(VisualAudioLab::maxNumberOfAudioDataChannels * sizeof(uint8 *));
	}
	for (i = 0; i < VisualAudioLab::maxNumberOfMusicDataHistory; i++)
	{
		for (k = 0; k < VisualAudioLab::maxNumberOfAudioDataChannels; k++)
		{
			spectrumData[i][k] =
				(uint8 *)malloc(VisualAudioLab::numberOfAudioSpectrumDataEntries * sizeof(uint8));
	...
	// amplitudeEnvelope
	amplitudeEnvelope =
		(double *)malloc(VisualAudioLab::numberOfAudioWaveformDataEntries * sizeof(double));
	...
	// beatGapHistogram
	beatGapHistogram =
		(uint32 *)malloc(VisualAudioLab::maxNumberOfBeatHistogramBins * sizeof(uint32));
	...
	// peakWaveformValSum
	peakWaveformValSum =
		(uint32 *)malloc(VisualAudioLab::maxNumberOfAudioDataChannels * sizeof(uint32));
	...
	beatsRecordedInBeatAnalyseTimeFrame = (BeatGuess *)malloc(
		VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe * sizeof(BeatGuess));
...
void VisualAudioLab::resetData(void)
{
	...
	{
		theVisualAudioLab->beatsRecordedInBeatAnalyseTimeFrame[i].timeInMillisecondsSinceStartOfTrack =
			0;
...
void VisualAudioLab::processAudioData(uint8 numWaveformChannels, uint16 numWaveformEntries,
	const uint8 *const currWaveformData, uint8 numSpectrumChannels, uint16 numSpectrumEntries,
...
void VisualAudioLab::processWaveformData(
	uint8 numWaveformChannels, uint16 numWaveformEntries, const uint8 *const currWaveformData)
{
	...
	elapsedMillisecondsSinceStartOfTrack =
		VisualTiming::getElapsedMilliSecsSinceReset("timeSinceStartOfTrack");

	elapsedMilliseconds = VisualTiming::getElapsedMilliSecsSinceLastCall("processWaveformData");

	sampleRate =
		static_cast<uint32>(static_cast<float>(VisualAudioLab::numberOfAudioWaveformDataEntries)
												* (1000.0 / static_cast<float>(elapsedMilliseconds)));
	...
	for (channel = 0; channel < numWaveformChannels; channel++)
	{
		...
		{
			theVisualAudioLab->mean[channel] +=
				(abs)(currWaveformData[channel * numWaveformEntries + index] - 128);
		}
	...
	for (channel = 0; channel < numWaveformChannels; channel++)
	{
		...
		for (index = 0; index < numWaveformEntries; index++)
		{
			...
			theVisualAudioLab->waveformData[theVisualAudioLab->currMusicDataHistory][channel][index] =
				waveformVal;

			if (channel == 0)
			{
				theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index] =
					waveformVal;
			}
			else
			{
			...
			theVisualAudioLab->rms[theVisualAudioLab->currMusicDataHistory][channel] +=
				(double)pow(static_cast<float>(abs(waveformVal)) - theVisualAudioLab->mean[channel], 2);
		...
		theVisualAudioLab->rms[theVisualAudioLab->currMusicDataHistory][channel] =
			sqrt(theVisualAudioLab->rms[theVisualAudioLab->currMusicDataHistory][channel]);

		sprintf(labelStr, "RMS channel %u", channel);
		sprintf(
			infoStr, "%10.3lf", theVisualAudioLab->rms[theVisualAudioLab->currMusicDataHistory][channel]);
		...
		theVisualAudioLab->avgWaveformValSum[channel] =
			((theVisualAudioLab->avgWaveformValSum[channel]
					 * ((theVisualAudioLab->avgWaveformValSumCounter[channel]) - 1)
				 + theVisualAudioLab->waveformValSum[channel])
				/ theVisualAudioLab->avgWaveformValSumCounter[channel]);
	}
	...
	for (index = 0; index < numWaveformEntries; index++)
	{
		...
		theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index] /=
			numWaveformChannels;
		theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory] += (double)pow(
			static_cast<float>(
				abs(theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index])),
			2);
		if (theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index]
				< theVisualAudioLab->minWaveformValMono)
		{
			theVisualAudioLab->minWaveformValMono =
				(sint8)theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index];
		}
		if (theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index]
				> theVisualAudioLab->maxWaveformValMono)
		{
			theVisualAudioLab->maxWaveformValMono =
				(sint8)theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index];
		}
	...
	theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory] =
		sqrt(theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory]);
	// sprintf(infoStr, "%4.4f",theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory]);
	// setProcessInfo ("rms_mono", infoStr);

	/*
		if (theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory] >
		theVisualAudioLab->rmsMonoMax) { theVisualAudioLab->rmsMonoMax =
		theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory];
		}
	*/

	theVisualAudioLab->rmsBuffer[theVisualAudioLab->currRMSValue].rmsValue =
		(float)theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory];
	theVisualAudioLab->rmsBuffer[theVisualAudioLab->currRMSValue]
		.timeInMillisecondsSinceStartOfTrack = elapsedMillisecondsSinceStartOfTrack;
	...
	{
		if ((theVisualAudioLab->rmsBuffer[index].timeInMillisecondsSinceStartOfTrack > 0.0)
				&& (theVisualAudioLab->rmsBuffer[index].timeInMillisecondsSinceStartOfTrack
						> (elapsedMillisecondsSinceStartOfTrack - numberOfMillisecondsOfRMSBuffer)))
		{
	...
	// theVisualAudioLab->rmsMonoAvg = (theVisualAudioLab->rmsMonoAvgSum +
	// theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory]) /
	// theVisualAudioLab->rmsMonoAvgCount;
	// sprintf(infoStr, "%10.3lf", theVisualAudioLab->rmsMonoAvg);
	// setProcessInfo ("rmsMono avg", infoStr);
	// theVisualAudioLab->rmsMonoAvgSum = theVisualAudioLab->rmsMonoAvgSum +
	// theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory]; sprintf(infoStr,
	// "%f",theVisualAudioLab->rmsMonoAvg); setProcessInfo ("rms_mono_avg", infoStr);
	...
	for (index = 0; index < numWaveformEntries; index++)
	{
		theVisualAudioLab->tappingBuffer[tappingBufferHighWatermarkIdx] = static_cast<double>(
			theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index]);
	...
	// sampleRate =
	// ((float)VisualAudioLab::numberOfAudioWaveformDataEntries/(float)elapsedMilliseconds * 1000.0);
	// sampleRate = (float)VisualAudioLab::numberOfAudioWaveformDataEntries * (1000.0 /
	...
	else
	{
		...
		numberOfSamplesToInterpolate =
			(uint16)((4000.0 * ((float)elapsedMilliseconds) / 1000.0) - numWaveformEntries);

		// cout << "elapsedMilliseconds: " << elapsedMilliseconds << endl;
		// cout << numberOfSamplesToInterpolate << " samples fehlen zur konstanten sample rate 6000" <<
		// endl;

		// add lost samples

		// linear interpoliert (?)
		lastWaveVal = static_cast<double>(
			theVisualAudioLab
				->waveformDataMono[theVisualAudioLab->currMusicDataHistory][numWaveformEntries - 1]);
		...
		{
			theVisualAudioLab->tappingBuffer[tappingBufferHighWatermarkIdx] =
				lastWaveVal + (interpolStepVal * (index + 1));
			tappingBufferHighWatermarkIdx++;
			numberOfTappingSamplesProcessed++;
			if (tappingBufferHighWatermarkIdx == tappingBufferLength)
			{
		...
		// cout << "elapsed: " << elapsedMilliseconds << " nulls: " << numberOfSamplesToInterpolate << "
		// count: " << numberOfSamplesToAnalyse << endl;
	}

	// numberOfSamplesToAnalyse = numberOfSamplesToAnalyse / 2; // pauschale Reduzierung der
	// Datenmenge (?)
...
void VisualAudioLab::addToBeatRecords()
{
	...
	theVisualAudioLab->beatsRecordedInBeatAnalyseTimeFrame[theVisualAudioLab->currRecordedBeat]
		.timeInMillisecondsSinceStartOfTrack = elapsedMilliseconds;
	theVisualAudioLab->beatsRecordedInBeatAnalyseTimeFrame[theVisualAudioLab->currRecordedBeat]
		.confidence = 0;
	theVisualAudioLab->currRecordedBeat++;
	if (theVisualAudioLab->currRecordedBeat
			== VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe)
	{
		theVisualAudioLab->currRecordedBeat = 0;
	}
}
...
void VisualAudioLab::calcBeatConfidence()
{
	...
	for (i = 0; i < VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe; i++)
	{
		...
		currBeatMSInterval =
			beatsRecordedInBeatAnalyseTimeFrame[i].timeInMillisecondsSinceStartOfTrack
			- beatsRecordedInBeatAnalyseTimeFrame[prevBeatIndex].timeInMillisecondsSinceStartOfTrack;
		if ((currBeatMSInterval < maximumBeatInterval) && (currBeatMSInterval > beatTimeTolerance))
		{
			for (j = 0; j < VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe; j++)
			{
				...
				checkedBeatMSInterval =
					beatsRecordedInBeatAnalyseTimeFrame[j].timeInMillisecondsSinceStartOfTrack
					- beatsRecordedInBeatAnalyseTimeFrame[prevBeatIndex].timeInMillisecondsSinceStartOfTrack;
				if ((checkedBeatMSInterval < (currBeatMSInterval + beatTimeTolerance))
						&& (checkedBeatMSInterval > (currBeatMSInterval - beatTimeTolerance)))
				{
...
uint32 VisualAudioLab::getBestBeatMSInterval()
{
	...
	{
		bestBeatMSInterval =
			beatsRecordedInBeatAnalyseTimeFrame[beatIndex].timeInMillisecondsSinceStartOfTrack
			- beatsRecordedInBeatAnalyseTimeFrame
					[VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe]
						.timeInMillisecondsSinceStartOfTrack;
	}
	else
	{
		bestBeatMSInterval =
			beatsRecordedInBeatAnalyseTimeFrame[beatIndex].timeInMillisecondsSinceStartOfTrack
			- beatsRecordedInBeatAnalyseTimeFrame[beatIndex - 1].timeInMillisecondsSinceStartOfTrack;
	}
...
uint8 VisualAudioLab::isBeatMeter(uint32 elapsedMSSinceStartOfTrack)
{
	...
	{
		if (beatsRecordedInBeatAnalyseTimeFrame[index].timeInMillisecondsSinceStartOfTrack
				< (elapsedMSSinceStartOfTrack + 100))
		{
			if (beatsRecordedInBeatAnalyseTimeFrame[index].timeInMillisecondsSinceStartOfTrack
					> (elapsedMSSinceStartOfTrack - 100))
			{
...
void VisualAudioLab::processSpectrumData(const uint8 numSpectrumChannels,
	const uint16 numSpectrumEntries, const uint8 *const currSpectrumData)
{
	uint16 index;
	sint32 channel;
	uint8 spectrumVal;
	...
	for (channel = 0; channel < numSpectrumChannels; channel++)
	{
		...
		for (index = 0; index < numSpectrumEntries; index++)
		{
			...
			theVisualAudioLab->spectrumData[theVisualAudioLab->currMusicDataHistory][channel][index] =
				spectrumVal;
...
{
	uint8 intensity;
	intensity = (uint8)((theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory]
												/ theVisualAudioLab->rmsMonoMax)
											* 100.0);
	return intensity;
}
...
{
	return theVisualAudioLab->waveformData[theVisualAudioLab->currMusicDataHistory][channel][index]
				 + 128;
}
...
/*
void VisualAudioLab::getMonoWaveformData(const uint8* const waveformArr, const uint8
numberOfHistories) { uint8 historyIndex; uint16 waveDataIndex; sint8 aHistory; if (numberOfHistories
> VisualAudioLab::maxNumberOfMusicDataHistory) return; aHistory =
theVisualAudioLab->currMusicDataHistory; for (historyIndex = 0; historyIndex < numberOfHistories;
historyIndex++) { aHistory++; if (aHistory == VisualAudioLab::maxNumberOfMusicDataHistory) {
						aHistory = 0;
				}
				for (waveDataIndex = 0; waveDataIndex < VisualAudioLab::numberOfAudioWaveformDataEntries;
waveDataIndex++) { waveformArr[aHistory * numberOfHistories + waveDataIndex] =
theVisualAudioLab->waveformDataMono[aHistory][waveDataIndex]; waveformArr[aHistory *
numberOfHistories + waveDataIndex] += 128;
						// + 128 (?)
				}
		}
}
...
uint8 VisualAudioLab::getWaveformDataOfChannelAtIndexWithHistoryNum(
	const uint16 channel, const uint16 index, const uint16 historyNum)
{
	return theVisualAudioLab->waveformData[historyNum][channel][index] + 128;
}

uint8 VisualAudioLab::getMonoWaveformDataAtIndexWithHistoryNum(
	const uint16 index, const uint16 historyNum)
{
...
void VisualAudioLab::HannWindow(const int pNumberOfWaveformEntries, double TimeSamples[])
{
	...
	{
		TimeSamples[i] =
			TimeSamples[i] * 0.5 * (1 - cos((2 * 3.1415926535 * i) / (pNumberOfWaveformEntries - 1)));
	}
}
...
void VisualAudioLab::storeBeatImpulse(bool beatImpulse)
{
	...
	if (beatImpulse)
	{
		theVisualAudioLab->beatTimeDeltaInMS =
			VisualTiming::getElapsedMilliSecsSinceReset("beatTimestamp");
...
uint8 VisualAudioLab::getBeatMeter()
{
	...
	// char infoStr[32];
	elapsedMilliseconds = VisualTiming::getElapsedMilliSecsSinceReset("timeSinceStartOfTrack");
	elapsedMillisecondsSinceLastBeatConfirmed =
		VisualTiming::getElapsedMilliSecsSinceReset("prevPositiveMeter");
