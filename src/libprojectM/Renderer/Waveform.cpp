...
Waveform::Waveform(int _samples) : RenderItem(),samples(_samples),points(_samples),pointContext(_samples){
spectrum=false;
dots=false;
thick=false;
additive=false;
...
glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,sizeof(ColoredPoint),(void*)0);    
glVertexAttribPointer(1,4,GL_FLOAT,GL_FALSE,sizeof(ColoredPoint),(void*)(sizeof(float)*2));   
}
...
if (dots){
glDrawArrays(GL_POINTS,0,samples_count);
}else{
glDrawArrays(GL_LINE_STRIP,0,samples_count);
}
glBindVertexArray(0);
