...
typedef enum
{
	...
	kStopValueHit,			/**< The set stop value is hit or crossed (depending on the current move
												 direction). */
...
public:
	 ...
	 * @return True if the new current value was outside of current min and max value range (and
	 * distance and duration were adjusted because of that).
	 * @remarks The value of the timeline can be set from the outside (e.g. in case it is created new
	 * and should start with a established value).
	 */
	...
	/**
	 * Static helper function that converts a TimelineUpdateResult to the string. Possibly useful for
	 ...
	 */
	static void convertTimelineUpdateResultToString(
