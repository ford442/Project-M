...
// Texture file links in a directory index. Keyed on the file extension rather
// than on the listing's layout: an Apache index opens with sort-column and
// "Parent Directory" links, which used to be skipped by starting at a magic
// index 5 that any other server (or index option) silently broke.
// (No regex literals in EM_JS bodies: the body is stringized, and a bare
// backslash does not survive that.)
var TEXTURE_EXTENSIONS = ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'tga', 'dds'];
function isTextureFile(href){
    var path = href.split('?')[0];
    var dot = path.lastIndexOf('.');
    return dot !== -1 && TEXTURE_EXTENSIONS.indexOf(path.slice(dot + 1).toLowerCase()) !== -1;
}
...
function textures(xml, textureBase){
    ...
    const links = htmlDocs.getElementsByTagName('a');
    $texs = [];
    for (var i = 0; i < links.length; i++) {
        var fname = links[i].getAttribute('href') || '';
        if (fname.charAt(0) === '?' || !isTextureFile(fname)) {
            continue;
        }
        var fullUrl = new URL(fname, textureBase).href;
        var fileName = decodeURIComponent(fullUrl.split('/').pop());
        $texs.push(fullUrl);
    ...
    console.log('scanned: ' + $texs.length + ' textures from ' + textureBase);
}
...
// Custom presets come from the same JSON manifest the preset picker reads
// (html/custom_presets_manifest.json, generated by
// scripts/generate_custom_preset_manifest.mjs) — not from scraping a directory
// index on one hard-coded host. The manifest URL can be overridden with a
// #customMilkManifest element; the preset base with #customMilkDir. Without
// one, each file is tried against the same bases projectm-preset-cache.js's
// defaultBasesForBase() uses, in order.
function customMilkBases(){
    var configured = getBasePath('#customMilkDir', '');
    if (configured) {
        return [configured];
    }
...
function customMilkShds(manifest){
    var presets = (manifest && Array.isArray(manifest.presets)) ? manifest.presets : [];
    var bases = customMilkBases();
    $customMilk = [];
    for (var i = 0; i < presets.length; i++) {
        var file = presets[i] && presets[i].file;
        if (typeof file !== 'string' || file.indexOf('.milk') === -1) {
            continue;
        }
        $customMilk.push({
            file: file,
            urls: bases.map(function(base){ return new URL(encodeURIComponent(file), new URL(base, globalThis.location.href)).href; }),
        });
    }
    console.log('Scanned ' + $customMilk.length + ' custom milk presets from the manifest.');
...
function scanCustomMilk(){
    // getBasePath() appends a '/', which a file URL does not want.
    var manifestUrl = getBasePath('#customMilkManifest', '');
    manifestUrl = manifestUrl ? manifestUrl.slice(0, -1) : 'custom_presets_manifest.json';
    fetch(new URL(manifestUrl, globalThis.location.href).href)
        .then(function(res){
            if (!res.ok) { throw new Error('HTTP ' + res.status); }
            return res.json();
        })
        .then(customMilkShds)
        .catch(function(err){ console.warn('Custom preset manifest unavailable (' + manifestUrl + '):', err); });
}

// Fetches the first of `urls` that answers, as an ArrayBuffer (null if none do).
function fetchFirst(urls){
    var attempt = function(i){
        if (i >= urls.length) { return Promise.resolve(null); }
        return fetch(urls[i])
            .then(function(res){ return res.ok ? res.arrayBuffer() : attempt(i + 1); })
...
function getCustomMilkShaders(){
    ...
    Promise.all($customMilk.map(function(entry, idx){
        return fetchFirst(entry.urls).then(function(buf){
            if (buf) {
                FS.writeFile('/presets/custmilk_' + idx + '.milk', new Uint8ClampedArray(buf));
                console.log('Got custom preset: custmilk_' + idx + '.milk (' + entry.file + ')');
            } else {
                console.warn('Failed to download custom preset: ' + entry.file);
            }
        });
    })).then(function(){
        console.log('Custom milk presets downloaded (' + total + '; not adding to auto-change playlist).');
...
var originalName = $customMilk[idx].file;
