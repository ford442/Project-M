 ...
 * Some virtual methods are pure abstract interface methods which must be overridden by the concrete
 * actor. Some virtual methods provide a default implementation which can be overridden by the
 * concrete actor.
 */
class VisualActor
{
...
public:
	 ...
	 * @remarks The initialization routine can be used to register for notifications by calling
	 * VisualNotification::registerNotification().
	 */
	virtual void init(void);

	/**
	 * Returns the name of the actor. The name is used as identifier by the VisualStageControl.
	 * @remarks Virtual method that does not need to be implemented by subclass. The name of the
	 * implemented visual actor is returned if the variable actorName is set correctly.
	 */
	 ...
	 * @remarks Show() is called by the VisualEnsemble as part of the showEnsemble() method. This
	 * interface method should be implemented by subclass to display the main graphic art show of the
	 * actor.
	 */
	 ...
	 * Each actor can register for a notification (event/message) by calling VisualNotification's
	 * registerNotification(). This function receives the notification. Hook method that can be
	 * implemented by subclass.
	 * @param aNotification The notification package.
	 * @remarks For debugging and diagnostical puposes, the subclass can explicitly call the method of
	 * the VisualActor interface class. Each notification key will be written to error log then.
	 */
	 ...
	 * This method is implemented by the VisualActor interface class. It calls the OpenGL error
	 * handler. Should not be overridden by subclass. Subclasses can implement the method but should
	 * then call the superclass implementation inside the subclass implementation also.
