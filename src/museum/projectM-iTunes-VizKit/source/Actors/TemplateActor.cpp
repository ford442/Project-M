...
TemplateActor::TemplateActor()
{
	...
	state = kVisActOn; // state must be kVisActOn to show the action of the actor as part of the
										 // VisualEnsemble (kVisActNoShow means not being called at all)
...
void TemplateActor::handleNotification(VisualNotification &aNotification)
{
	...
	switch (notificationKey)
	{
		...
		case kAudioPlayTrackChangedEvt: this->createStringOfSimilarArtists(); break;

		default:
			char notificationString[64];
			VisualNotification::convertNotificationKeyToString(notificationKey, notificationString);
			char errLog[256];
			sprintf(errLog, "Unhandled VisualNotificationKey %s in file: %s (line: %d) [%s])",
				notificationString, __FILE__, __LINE__, __FUNCTION__);
			writeLog(errLog);
			break;
	}
}
...
void TemplateActor::createStringOfSimilarArtists()
{
	...
	VisualString *normalizedArtistNameForURLConstruction =
		VisualNetwork::createNormalizedURLString(VisualDataStore::getArtistOfCurrentTrack());
	...
	this->stringOfSimilarArtists = VisualString::createWithContentsOfURL(
		similarArtistsURL.getUtf8Representation(), this->similarArtistsStringRequestId);
